...
if lev(0) gt lev(1) then begin &lev1=nlev-1 & lev2=0 & levstep=-1 & endif
for levi=lev1,lev2,levstep do begin ;loop through levels from top to bottom
        ...
        if keyword_set(lev_bnds) then begin
                ...
                case levi of 
                lev1:begin & d1=0 & d2=lev(0)&endcase ;layer thickness when derived from temps given at a discrete depth
                else:begin & d1=lev(levi-1) & d2=lev(levi) & endcase ;layer thickness when derived from temps given at a discrete depth
                endcase
        endelse
